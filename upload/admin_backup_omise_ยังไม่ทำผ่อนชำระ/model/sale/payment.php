...
class ModelSalePayment extends Model {
	...
	public function getPayment() {
		if(!empty($_GET['invoice_no'])) {
			$query = $this->db->query("SELECT * FROM fd_payment WHERE invoice_no = '".$_GET['invoice_no']."' ORDER BY payment_id DESC LIMIT 1");
		} else {
			$query = $this->db->query("SELECT * FROM fd_payment ORDER BY payment_id");
		}
	...
	public function getSlipPayment() {
		$query = $this->db->query("SELECT * FROM fd_payment ORDER BY invoice_no ASC");
