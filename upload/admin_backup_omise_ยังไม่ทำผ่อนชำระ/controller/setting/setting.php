...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			// insert email_receive

			//print_r($_POST['email_receive']);

			if(!empty($_POST['email_receive'])) {
				$this->db->query('TRUNCATE TABLE fd_email_receive');

				foreach($_POST['email_receive'] as $email) {
					$this->db->query("INSERT INTO fd_email_receive(email_receive, email_receive_datetime_create) VALUES('".$email."', '".date('Y-m-d H:i:s')."')");
				}
			}
			// end insert email_receive

			$this->model_setting_setting->editSetting('config', $this->request->post);

			// bank account
			if(!empty($_FILES['bank_account_image']['tmp_name'])) {
				if(move_uploaded_file($_FILES['bank_account_image']['tmp_name'], '../uploads_bank_account/'.$_FILES['bank_account_image']['name'])) {
					$path_and_file_image = "../uploads_bank_account/".$_FILES['bank_account_image']['name'];
					$this->db->query("UPDATE fd_bank_account SET bank_account_image = '".$path_and_file_image."', bank_account_company_name_th = '".$_POST['bank_account_company_name_th']."', bank_account_company_name_en = '".$_POST['bank_account_company_name_en']."', bank_account_account = '".$_POST['bank_account_account']."', bank_account_branch = '".$_POST['bank_account_branch']."', bank_account_branch = '".$_POST['bank_account_branch']."' WHERE bank_account_id = '1'");
				} else {
					$this->db->query("UPDATE fd_bank_account SET bank_account_company_name_th = '".$_POST['bank_account_company_name_th']."', bank_account_company_name_en = '".$_POST['bank_account_company_name_en']."', bank_account_account = '".$_POST['bank_account_account']."', bank_account_branch = '".$_POST['bank_account_branch']."', bank_account_branch = '".$_POST['bank_account_branch']."' WHERE bank_account_id = '1'");
				}
			} else {
				$this->db->query("UPDATE fd_bank_account SET bank_account_company_name_th = '".$_POST['bank_account_company_name_th']."', bank_account_company_name_en = '".$_POST['bank_account_company_name_en']."', bank_account_account = '".$_POST['bank_account_account']."', bank_account_branch = '".$_POST['bank_account_branch']."', bank_account_branch = '".$_POST['bank_account_branch']."' WHERE bank_account_id = '1'");
			}
		...
		$data['businessCtrl'] = $this->model_setting_setting->getBusinessHours();

		$data['email_receive'] = $this->model_setting_setting->getEmailReceive();

		$data['bank_account'] = $this->model_setting_setting->getBankAccount();
	...
	public function privacy() {
		...
		$data['action'] = 'index.php?route=setting/setting/privacy_save_update&user_token='.$this->session->data['user_token'];

		//echo $data['action'];

		$row = $this->db->query('Select * From fd_privacy Where privacy_id = 1');

		$data['privacy_detail'] = $row->row['privacy_detail'];

		$this->response->setOutput($this->load->view('setting/privacy_form', $data));
	}

	public function privacy_save_update() {
		//pre($_POST);

		$this->db->query('Update fd_privacy set privacy_detail = "'.$this->db->escape($_POST['privacy_detail']).'", privacy_datetime_update = "'.date('Y-m-d H:i:s').'" Where privacy_id = 1');

		$this->response->redirect($this->url->link('setting/setting/privacy', 'user_token=' . $this->session->data['user_token'], true));
	}
}
