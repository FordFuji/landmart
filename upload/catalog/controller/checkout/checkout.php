...
<?php	
			*/
			...
			if(!empty($_POST['installment_id'])) {
				$query_installment = $this->db->query("SELECT * FROM fd_installment WHERE installment_id = '".$_POST['installment_id']."'");
				$installment = $query_installment->row;
				
				//pre($installment);

				$who_installment_query = $this->db->query("SELECT * FROM fd_who_installment WHERE who_installment_id = 1");
				$whoInstallmentCtrl = $who_installment_query->row;

				if($whoInstallmentCtrl['who_installment_boolean'] == 'true') {
					$who_boolean = true;
				} elseif($whoInstallmentCtrl['who_installment_boolean'] == 'false') {
					$who_boolean = false;
				}
				...
				if(!empty($installment)) {
					...
					if($_SERVER['SERVER_NAME'] == 'localhost' or $_SERVER['SERVER_NAME'] == 'ford.orangeworkshop.info') {
						...
						$charge = OmiseCharge::create(array(
							'amount' => $order_total,
							'currency' => 'thb',
							'zero_interest_installments' => $who_boolean,
							'return_uri'  => 'http://localhost/landmart/upload/index.php?route=checkout/checkout/returnPaymentGateway&invoice_no='.$invoice_no,
							'source' => array(
								'type' => $installment['installment_name'],
								'installment_term' => $installment['installment_time']
							)
						));
	...
	public function returnPaymentGateway() {
		...
		if($charge['status'] != 'successful') {
			//echo "<h3 align='center'>บัตรเครดิตมีปัญหา ".$charge['failure_code'].' '.$charge['failure_message']."</h3>";
			echo "<h3 align='center'>บัตรเครดิตมีปัญหา ".$charge['failure_message']."</h3><p align='center'><a href='index.php'>กลับหน้าหลัก</a></p>";
		} else {
