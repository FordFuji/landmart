...
class ModelCatalogTransport extends Model {
	...
	public function addtransport() {
		$query = $this->db->query("INSERT INTO fd_transport (
			transport_name,
			transport_link,
			transport_datetime_create,
			transport_datetime_update
			)
			VALUES (
			'".$_POST['transport_name']."',
			'".$_POST['transport_link']."',
			'".date('Y-m-d H:i:s')."',
			'".date('Y-m-d H:i:s')."'
			)");
	}

	public function getTranSportRecord() {
		$query = $this->db->query("SELECT * FROM fd_transport WHERE transport_id = '".@$_GET['transport_id']."'");

		return $query->row;
	}

	public function edittransport($transport_id) {
		$query = $this->db->query("UPDATE fd_transport SET transport_name = '".$_POST['transport_name']."', transport_link = '".$_POST['transport_link']."', transport_datetime_update = '".date('Y-m-d H:i:s')."' WHERE transport_id = '".$transport_id."'");
	}
}
